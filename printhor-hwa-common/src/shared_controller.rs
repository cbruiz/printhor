...
impl<M: embassy_sync::blocking_mutex::raw::RawMutex + 'static, T: 'static> ControllerKind<M, T>
    for ControllerRef<M, T>
{
    type Type = T;
    type MutexType = M;
}
...
impl<M: embassy_sync::blocking_mutex::raw::RawMutex + 'static, T: 'static> ControllerRef<M, T> {
    ...
    where
        F: FnOnce(&mut <ControllerRef<M, T> as ControllerKind<M, T>>::Type) -> Result<(), ()>,
    {
    ...
    where
        F: FnOnce(&mut <ControllerRef<M, T> as ControllerKind<M, T>>::Type) -> Result<ROK, RKO>,
    {
        let mut h = self.h.r.borrow_mut();
        let x = h.as_mut();
        match x {
...
impl<M: embassy_sync::blocking_mutex::raw::RawMutex + 'static, T: 'static> Clone
    for ControllerRef<M, T>
{
    fn clone(&self) -> Self {
        ControllerRef {
